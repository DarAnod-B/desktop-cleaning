...
def saving_config_changes(values):
    change_STORAGE_FOLDER(
        values['-STORAGE_FOLDER-'])
    change_ALLOWED_EXTENSIONS(values['-ALLOWED_EXTENSIONS-'])
    change_EXCLUDED_NAMES(values['-EXCLUDED_NAMES-'])
...
def launch_interface():
    ...
    path_to_icon = os.path.join(os.getcwd(), 'setting', 'data', 'icon.ico')
    global window
